...
def regrid(ds_in, ds_out, method='bilinear', globe=True, periodic=True, reuse_weights=True):
    """
        ...
        >>> obs_regrid = u.regrid(obs, model, 'bilinear', globe=True, periodic=True, 
                                  reuse_weights=True)
...
def add_matrix_NaNs(regridder):
    """
        Add NaN values outside of the grid (otherwise 0 values are put by default
        in xESMF)
