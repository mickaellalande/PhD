...
def plot_ref_new_obs(
    ...
    """
        Plot 2 simulations versus observations. First row contains the 2 simulations 
        and their differences (diff). Second raw contains their bias regarding to 
        ...
        ref, new, obs : DataArray
            Reference, new simulation and observation. Needs to have the attributes
            'title' for the name of the plots and 'period' for the suptitle and
            saving the figure.
    ...
    m2 = axs[2].pcolormesh(new - ref, cmap=cmap_diff, levels=levels_diff, extend=extend_diff)
    ...
    m3 = axs[3].pcolormesh(ref - obs, cmap=cmap_bias, levels=levels_bias, extend=extend_bias)
    axs[3].format(title=ref.title+'\n - '+obs.obs_name)
    
    # Bias of new simulation regarding to observation
    axs[4].pcolormesh(new - obs, cmap=cmap_bias, levels=levels_bias, extend=extend_bias)
    ...
    if save:
        for extension in ['jpg', 'png', 'pdf']:
            fig.save(
                'img/'+var+'_'+new.title+'_REF_'+obs.obs_name+'_'+new.period+'.'+extension
            )
